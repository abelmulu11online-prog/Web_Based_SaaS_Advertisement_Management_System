...
export function ListingCard({ ad }) {
  ...
  // Public listing API returns `primary_image: {id, image_url}`.
  // Detail / my-ads endpoints return a full `images: []` array.
  // Support both shapes so this component works everywhere.
  const primary =
    ad.primary_image ||
    ad.images?.find(i => i.is_primary) ||
    ad.images?.[0] ||
    null
