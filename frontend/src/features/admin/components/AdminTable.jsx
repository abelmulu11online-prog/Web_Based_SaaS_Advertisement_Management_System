 ...
 *   BarChart       — horizontal CSS bar chart (with accessible data summary)
 *   Sparkline      — SVG polyline sparkline (aria-hidden, decorative)
 *   PageHeader     — consistent page title + subtitle + action
 *   EmptyState     — styled empty/no-results block
 *   ConfirmDialog  — accessible delete / destructive confirmation modal with focus trap
 *
 * Accessibility improvements:
 * - ConfirmDialog: useFocusTrap keeps focus inside; Escape calls onCancel;
 *   focus moves to Cancel button on open; focus returns to trigger on close.
 * - BarChart: visually-hidden <table> provides screen-reader accessible data.
 * - SearchBar: aria-label on input, clear button has aria-label.
 * - FilterSelect: aria-label on select.
 * - Pagination: aria-label on prev/next, aria-current="page" on current page.
 * - StatCard: trend indicator uses aria-label so screen readers announce
 *   the full meaning (not just the icon).
 */
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   - useFocusTrap keeps Tab/Shift+Tab inside the dialog
...
export function ConfirmDialog({
  ...
  // useFocusTrap moves focus to first focusable child (Cancel) when open=true
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p id="confirm-message" className="text-[13px] text-ink-2 leading-relaxed mb-6">{message}</p>
        <div className="flex gap-2.5 justify-end">
          {/* Cancel first — receives initial focus via useFocusTrap */}
          <button
            onClick={onCancel}
            className="
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   Accessibility: a visually-hidden <table> contains all data so screen
   readers can access the same information as sighted users.
════════════════════════════════════════════════════════════════════════════ */
