...
const VK = {
  list:   (p) => ['admin', 'verifications', p],
  detail: (id) => ['admin', 'verification', id],
  stats:  ['admin', 'verification-stats'],
}

export function useAdminVerifications(params = {}) {
  return useQuery({
    queryKey: VK.list(params),
    queryFn:  () => api.getProfileVerifications(params),
  })
}

export function useAdminVerificationDetail(profileId) {
  return useQuery({
    queryKey: VK.detail(profileId),
    queryFn:  () => api.getProfileVerificationDetail(profileId),
    enabled:  !!profileId,
  })
}
...
export function useApproveVerification() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ profileId, note }) => api.approveProfileVerification(profileId, note),
    onSuccess: (_data, { profileId }) => {
      qc.invalidateQueries({ queryKey: ['admin', 'verifications'] })
      qc.invalidateQueries({ queryKey: VK.detail(profileId) })
...
export function useRejectVerification() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ profileId, reason }) => api.rejectProfileVerification(profileId, reason),
    onSuccess: (_data, { profileId }) => {
      qc.invalidateQueries({ queryKey: ['admin', 'verifications'] })
      qc.invalidateQueries({ queryKey: VK.detail(profileId) })
      qc.invalidateQueries({ queryKey: VK.stats })
...
export function useSuspendVerification() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ profileId, reason }) => api.suspendProfileVerification(profileId, reason),
    onSuccess: (_data, { profileId }) => {
      qc.invalidateQueries({ queryKey: ['admin', 'verifications'] })
      qc.invalidateQueries({ queryKey: VK.detail(profileId) })
      qc.invalidateQueries({ queryKey: VK.stats })
