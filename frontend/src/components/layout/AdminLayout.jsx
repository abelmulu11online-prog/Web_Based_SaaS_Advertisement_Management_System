...
import {
  LayoutDashboard, Users, Megaphone, CreditCard, LogOut,
  Menu, X, ChevronLeft,
} from 'lucide-react'
  ...
  const Sidebar = () => (
    <div className="flex flex-col h-full">
      <div className="px-5 py-4 border-b border-border">
        ...
        <p className="text-[10px] text-ink-3 font-semibold uppercase tracking-widest mt-0.5">Admin Panel</p>
      </div>
    ...
    <div className="min-h-screen bg-canvas flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-surface border-b border-border px-4 sm:px-6 h-14 flex items-center gap-3 sticky top-0 z-40">
          <button onClick={() => setOpen(true)} className="lg:hidden text-ink-2 hover:text-ink" aria-label="Open menu">
            <Menu size={18} />
          </button>
          <h1 className="text-[15px] font-semibold text-ink flex-1">{title}</h1>
          <span className="text-[11px] font-semibold bg-danger text-white px-2 py-0.5 rounded uppercase tracking-widest">
            Admin
          </span>
        </header>
