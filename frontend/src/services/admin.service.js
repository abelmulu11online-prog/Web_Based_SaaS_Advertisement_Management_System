...
export async function deleteUser(userId) {
  const r = await apiClient.delete(`/admin/users/${userId}`)
  return r.data.data
}
...
export async function getProfileVerifications(params = {}) {
  const r = await apiClient.get('/admin/profile-verifications', { params })
  return r.data.data
}

export async function getProfileVerificationDetail(profileId) {
  const r = await apiClient.get(`/admin/profile-verifications/${profileId}`)
  return r.data.data
}

export async function approveProfileVerification(profileId, note = null) {
  const r = await apiClient.post(`/admin/profile-verifications/${profileId}/approve`, { note })
  return r.data.data
}

export async function rejectProfileVerification(profileId, reason) {
  const r = await apiClient.post(`/admin/profile-verifications/${profileId}/reject`, { reason })
  return r.data.data
}

export async function suspendProfileVerification(profileId, reason = null) {
  const r = await apiClient.post(`/admin/profile-verifications/${profileId}/suspend`, { reason })
  return r.data.data
}

export async function getVerificationStats() {
  const r = await apiClient.get('/admin/profile-verifications/stats')
  return r.data.data
}
