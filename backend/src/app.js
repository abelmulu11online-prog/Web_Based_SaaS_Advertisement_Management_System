...
// ── Routes that need raw body MUST come before express.json() ─────────────────
// The subscriptions webhook route uses express.raw() inline so it gets the
// raw Buffer for HMAC-SHA256 signature verification. If express.json() runs
// first, the raw bytes are consumed and the HMAC computation will differ from
// what Chapa sent — every webhook would be rejected as invalid.
app.use('/api/subscriptions', subscriptionsRouter)

// ── Body parsing (after raw-body routes) ─────────────────────────────────────
