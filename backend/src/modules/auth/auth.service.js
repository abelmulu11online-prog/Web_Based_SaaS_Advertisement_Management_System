...
export async function register({ email, phone, password }) {
  ...
  try {
    ...
    // If email provided, create and send verification token.
    // We track whether the email was actually sent so we can inform the caller.
    let emailSent = false
    if (normalizedEmail) {
      try {
        emailSent = await createAndSendVerificationToken(user.id, normalizedEmail, client)
      } catch (err) {
        // Token creation/DB error — log but don't roll back the registration
        logger.warn({ userId: user.id, error: err.message }, 'Failed to create verification token during registration')
      }
    }

    await client.query('COMMIT')
    // Return safe user data (no password_hash) + email send status for the controller
    return {
      id: user.id,
      email: user.email,
      phone: user.phone,
      ...
      emailSent,  // true = delivered, false = failed or no email provided
    }
  } catch (err) {
