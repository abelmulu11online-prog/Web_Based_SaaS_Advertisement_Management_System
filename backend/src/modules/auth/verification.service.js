...
export async function createAndSendVerificationToken(userId, email, dbClient = pool) {
  ...
  // Send verification email — awaited so callers know if delivery succeeded.
  // The email.service already catches all SMTP errors and returns false,
  // so this will never throw. We return the boolean result to the caller.
  ...
  if (!emailSent) {
    logger.warn({ userId, recipientDomain: email?.split('@')[1] }, 'Verification email delivery failed')
  }
