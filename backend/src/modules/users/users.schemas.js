...
export const updateProfileSchema = z.object({
  body: z
    .object({
      ...
      // Business verification fields (migration 036)
      business_name:    nullableStr(200),
      business_type:    nullableStr(100),
      license_number:   nullableStr(100),
      license_issue_date:  z.union([z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be YYYY-MM-DD'), z.null()]).optional(),
      license_expiry_date: z.union([z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be YYYY-MM-DD'), z.null()]).optional(),
      business_address: nullableStr(300),
      business_city:    nullableStr(100),
      business_region:  nullableStr(100),
      business_country: nullableStr(100),
      additional_information: z.union([z.string().max(3000, 'Additional information must not exceed 3000 characters'), z.null()]).optional(),
      ...rejectedFields,
    })
