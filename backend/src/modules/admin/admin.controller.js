...
export const deleteUser = asyncHandler(async (req, res) => {
  await service.deleteUser(req.params.userId, req.user.id)
...
export const listProfileVerifications = asyncHandler(async (req, res) => {
  const result = await service.listProfileVerifications(req.query || {})
  sendSuccess(res, 'Profile verifications retrieved', result)
})

export const getProfileVerificationDetail = asyncHandler(async (req, res) => {
  const result = await service.getProfileVerificationDetail(req.params.profileId)
  sendSuccess(res, 'Profile verification detail retrieved', result)
})

export const approveProfile = asyncHandler(async (req, res) => {
  const result = await service.approveProfile(req.params.profileId, req.user.id, {
    note: req.body?.note || null,
  })
  sendSuccess(res, 'Profile approved', result)
})

export const rejectProfile = asyncHandler(async (req, res) => {
  const result = await service.rejectProfile(req.params.profileId, req.user.id, {
    reason: req.body?.reason,
  })
  sendSuccess(res, 'Profile rejected', result)
})

export const suspendProfile = asyncHandler(async (req, res) => {
  const result = await service.suspendProfile(req.params.profileId, req.user.id, {
    reason: req.body?.reason || null,
  })
  sendSuccess(res, 'Profile suspended', result)
})

export const getVerificationStats = asyncHandler(async (_req, res) => {
  const result = await service.getVerificationStats()
  sendSuccess(res, 'Verification stats retrieved', result)
})
