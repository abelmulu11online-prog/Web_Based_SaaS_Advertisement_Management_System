...
// GET    /api/admin/profile-verifications          — list profiles under review
// GET    /api/admin/profile-verifications/stats    — count under-review profiles
// GET    /api/admin/profile-verifications/:id      — full detail for one profile
// POST   /api/admin/profile-verifications/:id/approve  — approve profile
// POST   /api/admin/profile-verifications/:id/reject   — reject with reason
// POST   /api/admin/profile-verifications/:id/suspend  — suspend active profile
...
const verificationListQuerySchema = z.object({
  query: z.object({
    ...
    page_size:    z.coerce.number().int().positive().max(100).optional(),
    search:       z.string().max(200).optional(),
    status:       z.enum(['UNDER_REVIEW','ACTIVE','REJECTED','SUSPENDED','UNVERIFIED','PENDING','VERIFIED']).optional(),
...
const approveSchema = z.object({
  ...
  body:   z.object({ note: z.string().max(1000).optional() }).optional(),
})

const rejectSchema = z.object({
  params: z.object({ profileId: z.string().uuid() }),
  body:   z.object({ reason: z.string().min(1, 'Rejection reason is required').max(2000) }),
})
...
router.get('/profile-verifications',                      ...guard, validate(verificationListQuerySchema),            ctrl.listProfileVerifications)
router.get('/profile-verifications/stats',                ...guard,                                                   ctrl.getVerificationStats)
router.get('/profile-verifications/:profileId',           ...guard, validate(z.object({ params: z.object({ profileId: z.string().uuid() }) })), ctrl.getProfileVerificationDetail)
router.post('/profile-verifications/:profileId/approve',  ...guard, validate(approveSchema),                         ctrl.approveProfile)
router.post('/profile-verifications/:profileId/reject',   ...guard, validate(rejectSchema),                          ctrl.rejectProfile)
router.post('/profile-verifications/:profileId/suspend',  ...guard, validate(suspendSchema),                         ctrl.suspendProfile)
