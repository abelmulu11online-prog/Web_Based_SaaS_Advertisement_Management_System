...
export async function listPublished(query) {
  ...
  const { rows, total } = await adsRepo.findPublished({
    ...
    // Geo-radius (passed through only when all three are present)
    lat: query.lat !== undefined ? query.lat : undefined,
    lng: query.lng !== undefined ? query.lng : undefined,
    radius_km: query.radius_km !== undefined ? query.radius_km : undefined,
    page,
    page_size: pageSize,
  })
  ...
  const advertisementsWithImages = await Promise.all(
    rows.map(async (row) => {
      ...
      return {
        ...formatAd(row),
        // Include distance when geo search was active
...
/**
 * Return minimal map-pin data for all published ads with coordinates.
 * Safe to expose publicly — only shows what advertisers intentionally provided.
 * @returns {Promise<object[]>}
 */
export async function listMapPins() {
  const rows = await adsRepo.findPublishedWithCoords(500)
  return rows.map((row) => ({
    id: row.id,
    title: row.title,
